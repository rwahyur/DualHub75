...
*/
  ...
  bus_cfg.pin_d7 = m_cfg.gpio.lat2;
  bus_cfg.pin_d8 = m_cfg.gpio.lat3;
  bus_cfg.pin_d9 = m_cfg.gpio.lat4;

  bus_cfg.pin_d10 = m_cfg.gpio.oe;  

  bus_cfg.pin_d11 = m_cfg.gpio.a;
  bus_cfg.pin_d12 = m_cfg.gpio.b;
  bus_cfg.pin_d13 = m_cfg.gpio.c;
  bus_cfg.pin_d14 = m_cfg.gpio.d;
  bus_cfg.pin_d15 = m_cfg.gpio.e;
  ...
  do
  {
    ...
    do
    {
      ...
      if (m_cfg.driver == HUB75_I2S_CFG::SM5266P)
      {
        ...
        row[x_pixel] = abcde & (18 << BITS_ADDR_OFFSET); // mask out the bottom 3 bits which are the clk di bk inputs
      }
      else if (m_cfg.driver == HUB75_I2S_CFG::DP3246_SM5368) 
      {
        row[ESP32_TX_FIFO_POSITION_ADJUST(x_pixel)] = 00000;
      }
      else
      {
    ...
    do
    {
      ...
      if (m_cfg.driver == HUB75_I2S_CFG::SM5266P)
      {
        // modifications here for row shift register type SM5266P
        // https://github.com/mrfaptastic/ESP32-HUB75-MatrixPanel-I2S-DMA/issues/164
        row[x_pixel] = abcde & (0x8 << BITS_ADDR_OFFSET); // mask out the bottom 3 bits which are the clk di bk inputs
      }
      else if (m_cfg.driver == HUB75_I2S_CFG::DP3246_SM5368) 
      {
        row[ESP32_TX_FIFO_POSITION_ADJUST(x_pixel)] = 0x1000;
      }
      else
      {
        row[ESP32_TX_FIFO_POSITION_ADJUST(x_pixel)] = abcde;
    ...
    if (m_cfg.driver == HUB75_I2S_CFG::SM5266P)
    {
      ...
      uint16_t latch;
      x_pixel = fb->rowBits[row_idx]->width - 016; // come back 8*2 pixels to allow for 8 writes
      serialCount = 8;
      do
      {
        serialCount--;
    ...
    if (m_cfg.driver == HUB75_I2S_CFG::DP3246_SM5368) 
    {
      ...
      row[ESP32_TX_FIFO_POSITION_ADJUST(x_pixel + 12)] |= c;                                                             // set row data
    ...
    do
    {
      ...
      if (m_cfg.driver == HUB75_I2S_CFG::DP3246_SM5368) 
      {
        row[ESP32_TX_FIFO_POSITION_ADJUST(fb->rowBits[row_idx]->width - 32)] |= BIT_LAT;   // DP3246 needs 3 clock cycle latch 
        row[ESP32_TX_FIFO_POSITION_ADJUST(fb->rowBits[row_idx]->width - 128)] |= BIT_LAT;   // DP3246 needs 3 clock cycle latch 
      } // DP3246_SM5368
  
   
uint16_t sr[0b100]={0100,0200,0400,01000},
lt=PIXELS_PER_ROW/tingkat; 

for(register uint8_t z=0;z<tingkat;z++){
uint16_t ind=(lt*z)+1;
row[ESP32_TX_FIFO_POSITION_ADJUST(fb->rowBits[row_idx]->width - ind)] |= sr[z]; 
}
...
frameStruct *fb = &frame_buffer[_buff_id];
  ...
  do {
    ...
    uint8_t coloridx = fb->rowBits[row_idx]->colour_depth;
    do {
      --coloridx;

      // switch pointer to a row for a specific color index
      ESP32_I2S_DMA_STORAGE_TYPE* row = fb->rowBits[row_idx]->getDataPtr(coloridx, _buff_id);

      int x_coord = fb->rowBits[row_idx]->width;
      do {
        --x_coord;
       ...
       // clear OE bit for all other pixels
        ...
        if((coloridx > lsbMsbTransitionBit || !coloridx) && ( (x_coord >= brt) /*|| (x_coord==999/&&x_coord<=1/)*/ ) ){
...
int8_t lat2, int8_t lat3, int8_t lat4)
{
  if (initialized)
    return true;
  // RGB
...
m_cfg.gpio.lat2 = lat2;
m_cfg.gpio.lat3 = lat3;
m_cfg.gpio.lat4 = lat4;
